...
def run_streamlit_app():
    ...
    if dna_sequence:
        # Option to choose between color map methods
        method = st.selectbox("Choose Barcode Method", ["DNA to Color", "Codon Color Barcode"])

        # Generate barcode based on selected method
        output_file = "dna_barcode.png"

        if method == "DNA to Color":
            generate_barcode(dna_sequence, output_file)
        elif method == "Codon Color Barcode":
...
st.header("Step 1: Upload Barcode Image for Prediction")
